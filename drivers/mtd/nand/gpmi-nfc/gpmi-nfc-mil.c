...
static int mil_scan_bbt(struct mtd_info *mtd)
{
	...
	if (!info) {
		pr_err("Unrecognized NAND Flash device.gpmi-nfc-mil.\n");
		return !0;
	}
...
static int mil_boot_areas_init(struct gpmi_nfc_data *this)
{
	...
	unsigned int                   i;
	struct mtd_partition           partitions[7];
	...
	//static char  *general_use_name      = "gpmi-nfc-general-use1";
	static char  *general_use_name      = "rootfs   64MB";
 
    //add by yufangmin
    static char *str_uboot = "reserve";
    static char *str_reserve= "reserve";
    static char *str_log = "bmp";
    //static char *str_zlgpart = "zlgpart";
    //sstatic char *str_zlgpart = "gpmi-nfc-general-use";
    //static char *str_zlgpart = "rootfs";
	...
	if (rom->boot_area_count == 1) {
        ...
        // add by yufangmin
        partitions[0].name       = "boot     12MB";
        partitions[0].offset     = 0;
        partitions[0].size       = 12*1024*1024;
        partitions[0].mask_flags = 0;

        partitions[1].name       = "env      512KB";
        partitions[1].offset     = MTDPART_OFS_APPEND;
        partitions[1].size       = 512*1024;
        partitions[1].mask_flags = 0;

        partitions[2].name       = "reserved 512KB";
        partitions[2].offset     = MTDPART_OFS_APPEND;
        partitions[2].size       = 512*1024;
        partitions[2].mask_flags = 0;

        partitions[3].name       = "logo     2MB";
...
//        partitions[6].size       = 168*1024*1024; 
		...
		add_mtd_partitions(mtd, partitions, 7);
